    ...
    public int scan(byte[] input, int offset, int length, @NotNull ScanResult out) {
        ...
        while (i < simdLimit) {
            ...
            while (i < chunkEnd) {
                ...
                // Spill upper bits if vector straddles word boundary
                if (shift != 0) {
        ...
        // Handle tail bytes
        if (tail > 0) {
            final ByteVector vec = ByteVector.fromArray(SPECIES, input, offset + i, SPECIES.indexInRange(0, tail));
            final long laneMask = LANE_MASKS[tail];
            ...
            long escaped    = computeEscapedPositions(bsMask, prevBackslashOdd);
            long toggles    = qMask & ~escaped;
            long inStrAfter = (parallelPrefixXor(toggles) ^ (prevInString != 0 ? -1L : 0L)) & laneMask;
            long inStrMask  = inStrAfter & ~toggles;
            ...
            // Spill upper bits if tail crosses word boundary
            if (shift + tail > 64) {
                out.quoteMask[word+1]        |= toggles >>> (64 - shift);
                out.backslashMask[word+1]    |= bsMask >>> (64 - shift);
                out.controlMask[word+1]      |= ctrlMask >>> (64 - shift);
                out.structuralMask[word+1]   |= (strMask & ~inStrMask) >>> (64 - shift);
                out.insideStringMask[word+1] |= inStrMask >>> (64 - shift);
            }

            // prevInString = last byte of the tail
            prevInString = ((inStrMask >>> (tail - 1)) & 1L) != 0 ? 1L : 0L;
